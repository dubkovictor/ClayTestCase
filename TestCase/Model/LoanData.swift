...
struct LoanDetails: Codable {
    ...
    var interestPaidToDate1: String {
        return "\(interestPaidToDate)".components(separatedBy: ".").first ?? ""
    }
    
    var interestPaidToDate2: String {
        return "\(interestPaidToDate)".components(separatedBy: ".").last ?? ""
    }
}
...
struct PaymentDetails: Codable {
    ...
    var toDataFormat: String {
        let oldDateFormatter = DateFormatter()
        oldDateFormatter.dateFormat = "yyyy-MM-dd"
        
        let oldDate = oldDateFormatter.date(from: nextPaymentDate)
    ...
    var amountDue1: String {
        return "\(amountDue)".components(separatedBy: ".").first ?? ""
    }
    
    var amountDue2: String {
        return "\(amountDue)".components(separatedBy: ".").last ?? ""
    }
}

// MARK: - PaymentProgress
